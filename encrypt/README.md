...
5. 在发行方电脑的python环境中运行如下指令，获得license.lic
    ...
    ```
6. 在目标宿主机上运行`host_attest_gen.py`生成`host_attest.json`

7. 在目标机器上重新挂载docker，增加license和host_attest挂载：
    ```bash
    docker run -it --name <your_container_name> -v /etc/machine-id:/host/etc/machine-id -v /sys/class/dmi/id:/host/sys/class/dmi/id:ro -v <your_license.lic>:/app/license.lic -v <your_host_attest.json>:/host/attest/host_attest.json <your_image:tag> /bin/bash
    ```
8. 在目标机器上的docker中的python中运行测试程序：
    ```python
    # issuer_public_key_b64：公钥，
    # license_master_key_b64 ：密钥
    # app_secret_b64：密钥，与license_master_key_b64一致
