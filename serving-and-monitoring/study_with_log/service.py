# service_main.py
# 带有log的服务的主入口,为他人提供接口，或者被动接受他人的回调
...
import requests
import logging
from logging_middleware import LoggingMiddleware,LoggingAuthMiddleware
...
app.add_middleware(LoggingAuthMiddleware)

# ---------------------
# 3. 接收外部调用的接口
# ---------------------
@app.post("/run_algo")
async def run_algo_endpoint(input_data: algo_input, request:Request):
    '''
    接收外部调用，运行算法并返回结果；
    ...
    client_host = request.state.client_host if hasattr(request.state, 'client_host') else "unknown"
    # 打印一条日志，记录一下谁来调用的，传了什么数据
    logger.info(f"requestReceived request from {client_host} with data: {input_data}")

    # 调用核心算法
    try:
        result = run_algo(input_data.x, input_data.y, input_data.z)
    except Exception as e:
        logger.exception(f"[run_algo]算法执行错误: {e}")
